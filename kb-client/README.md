...
| `case-fetch.py` | Read commands — `case` / `journey` / `list` / `fireside` / `library` / `edges` / `flags` / `read <TYPE>` (generic typed read, symmetric to the write path) / `search` (full-text over the whole corpus, kb + library as one) / `topics` (the vocabulary the validated `topics:` field must be drawn from) — over the gateway (CASE-393/479/482/518/630/683/707). |
